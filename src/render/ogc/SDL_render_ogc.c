...
static bool OGC_CreateTexture(SDL_Renderer *renderer, SDL_Texture *texture, SDL_PropertiesID create_props)
{
    u32 texture_size;
...
static bool OGC_QueueSetViewport(SDL_Renderer *renderer, SDL_RenderCommand *cmd)
{
                             ...
                             float scale_x, float scale_y)
{
    ...
    size_per_element = sizeof(SDL_FPoint) + sizeof(SDL_FColor);
    if (texture) {
    ...
    for (i = 0; i < count; i++) {
        ...
        col = *(SDL_FColor *)((char *)color + j * color_stride);
        ...
        vertex_xy->y = xy_[1] * scale_y;

        *(SDL_FColor *)(vertex + sizeof(SDL_FPoint)) = col;
...
static bool OGC_RenderSetClipRect(SDL_Renderer *renderer, SDL_RenderCommand *cmd)
{
    ...
    if (cmd->data.cliprect.enabled) {
        GX_SetScissor(renderer->view->viewport.x + rect->x,
...
static bool OGC_RenderClear(SDL_Renderer *renderer, SDL_RenderCommand *cmd)
{
    OGC_RenderData *data = renderer->internal;
    int w, h;
    ...
    GXColor c = {
        (u8)(cmd->data.color.color.r * 255.0f),
        (u8)(cmd->data.color.color.g * 255.0f),
        (u8)(cmd->data.color.color.b * 255.0f),
        (u8)(cmd->data.color.color.a * 255.0f)
    };
    int16_t x1 = 0;
    int16_t y1 = 0;
    int16_t x2 = w;
...
static bool OGC_RenderGeometry(SDL_Renderer *renderer, void *vertices,
...
static bool OGC_RenderPrimitive(SDL_Renderer *renderer, u8 primitive,
                        void *vertices, SDL_RenderCommand *cmd)
{
    ...
    GXColor c = {
        (u8)(cmd->data.draw.color.r * 255.0f),
        (u8)(cmd->data.draw.color.g * 255.0f),
        (u8)(cmd->data.draw.color.b * 255.0f),
        (u8)(cmd->data.draw.color.a * 255.0f)
    };
...
static SDL_Surface *OGC_RenderReadPixels(SDL_Renderer *renderer, const SDL_Rect *rect)
{
    return NULL;
}
...
static bool OGC_SetVSync(SDL_Renderer *renderer, int vsync)
{
    OGC_RenderData *data = (OGC_RenderData *)renderer->internal;
    data->vsync = vsync ? true : false;
    return true;
}

static bool OGC_CreateRenderer(SDL_Renderer *renderer, SDL_Window *window, SDL_PropertiesID create_props)
{
    OGC_RenderData *data;
