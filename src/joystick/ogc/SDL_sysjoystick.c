...
static void report_joystick(int index, int connected)
{
    ...
    if (connected) {
        s_connected_instances[index] = SDL_GetNextObjectID();
...
#ifdef __wii__
    s_accelerometers_as_axes = true;  // Default to enabled
...
{
    // OGC joysticks are always present if detected, we don't track by VID/PID
    return false;
}
...
static const char *OGC_JoystickGetDevicePath(int device_index)
{
    return NULL;
}
...
static bool OGC_JoystickSendEffect(SDL_Joystick *joystick,
...
static void HandleWiiHats(Uint64 timestamp,
...
static void HandleWiiButtons(Uint64 timestamp,
                             SDL_Joystick *joystick,
                             const u32 changed,
                             const WPADData *data,
                             const u32 *buttons,
                             size_t num_buttons)
{
    for (int i = 0; i < num_buttons; i++) {
        if (changed & buttons[i]) {
            ...
            SDL_SendJoystickButton(timestamp, joystick, i,
                                      (data->btns_d & buttons[i]) ? true : false);
        }
    }
}

static void HandleWiiMotion(Uint64 timestamp,
                            SDL_Joystick *joystick,
                            ...
                            int start_index)
{
    ...
    if (prev_state->wiimote.wiimote_pitch != axis) {
        SDL_SendJoystickAxis(timestamp, joystick, start_index, -(axis << 8));
        prev_state->wiimote.wiimote_pitch = axis;
    }
    axis = WPAD_Roll(data);
    if (prev_state->wiimote.wiimote_roll != axis) {
        SDL_SendJoystickAxis(timestamp, joystick, start_index + 1, axis << 8);
        prev_state->wiimote.wiimote_roll = axis;
    }
    axis = WPAD_Yaw(data);
    if (prev_state->wiimote.wiimote_yaw != axis) {
        SDL_SendJoystickAxis(timestamp, joystick, start_index + 2, axis << 8);
        prev_state->wiimote.wiimote_yaw = axis;
    }
}

static void HandleNunchuckSensors(Uint64 timestamp,
                                  SDL_Joystick *joystick,
...
static void HandleWiimoteSensors(Uint64 timestamp,
                                 SDL_Joystick *joystick,
                                 WPADData *data)
{
    float values[3];
    ...
    values[1] = data->gforce.z * SDL_STANDARD_GRAVITY;
    values[2] = -data->gforce.y * SDL_STANDARD_GRAVITY;
    SDL_SendJoystickSensor(timestamp, joystick, SDL_SENSOR_ACCEL, 0, values, 3);
}
...
static void _HandleWiiJoystickUpdate(SDL_Joystick *joystick)
{
    ...
    if (update_wiimote) {
        HandleWiiHats(timestamp, joystick, changed, pressed, _buttons);
    }
    if (update_expansion) {
        if (data->exp.type == WPAD_EXP_CLASSIC) {
            HandleWiiHats(timestamp, joystick, changed, pressed, _buttons + 4);
        }
    }

    if (split_joysticks) {
        if (update_wiimote) {
            HandleWiiButtons(timestamp, joystick, changed, data,
    ...
    if (update_expansion) {
        ...
        if (data->exp.type != prev_state->wiimote.exp) {
            ...
            for (int i = 0; i < 6; i++)
                SDL_SendJoystickAxis(timestamp, joystick, i, 0);
        }
        ...
        if (data->exp.type == WPAD_EXP_CLASSIC) {
            ...
            if (prev_state->wiimote.classicL_stickX != axis) {
                SDL_SendJoystickAxis(timestamp, joystick, 0, axis);
            ...
            if (prev_state->wiimote.classicL_stickY != axis) {
                SDL_SendJoystickAxis(timestamp, joystick, 1, axis);
            ...
            if (prev_state->wiimote.classicR_stickX != axis) {
                SDL_SendJoystickAxis(timestamp, joystick, 2, axis);
            ...
            if (prev_state->wiimote.classicR_stickY != axis) {
                SDL_SendJoystickAxis(timestamp, joystick, 3, axis);
                prev_state->wiimote.classicR_stickY = axis;
            }
            axis = (int)data->exp.classic.r_shoulder;
            if (prev_state->wiimote.classic_triggerR != axis) {
                SDL_SendJoystickAxis(timestamp, joystick, 4, axis << 8);
                prev_state->wiimote.classic_triggerR = axis;
            }
            axis = (int)data->exp.classic.l_shoulder;
            if (prev_state->wiimote.classic_triggerL != axis) {
                SDL_SendJoystickAxis(timestamp, joystick, 5, axis << 8);
            ...
            HandleNunchuckSensors(timestamp, joystick, &data->exp.nunchuk);
        }
    }
    ...
    if (update_wiimote) {
        if (s_accelerometers_as_axes) {
            ...
            HandleWiiMotion(timestamp, joystick, prev_state, data, start_index);
        }
        HandleWiimoteSensors(timestamp, joystick, data);
    }
}
#endif /* __wii__ */
...
static bool OGC_JoystickGetGamepadMapping(int device_index,
...
SDL_JoystickDriver SDL_OGC_JoystickDriver = {
    ...
    .IsDevicePresent = OGC_JoystickIsDevicePresent,
    .GetDeviceName = OGC_JoystickGetDeviceName,
    .GetDevicePath = OGC_JoystickGetDevicePath,
    .GetDeviceSteamVirtualGamepadSlot = OGC_JoystickGetDeviceSteamVirtualGamepadSlot,
