...
static void draw_cursor_rect(SDL_CursorData *curdata)
{
    draw_rect(-curdata->hot_x, -curdata->hot_y, curdata->w, curdata->h);
}

static void setup_2d_viewport(SDL_VideoDevice *_this)
{
...
static SDL_Cursor *OGC_CreateCursor(SDL_Surface *surface, int hot_x, int hot_y)
{
    ...
    rect.w = surface->w;
    rect.h = surface->h;
    OGC_pixels_to_texture(surface->pixels, surface->format, &rect,
                          surface->pitch, curdata->texels, surface->w);
    DCStoreRange(curdata->texels, texture_size);
    GX_InvalidateTexAll();

    /* assign the driver cursor data to the core's opaque pointer */
...
SDL_Cursor *OGC_CreateSystemCursor(SDL_SystemCursor id)
{
    ...
    surface =
        SDL_CreateSurfaceFrom((int)cursor->width,
                             (int)cursor->height,
                             SDL_PIXELFORMAT_RGBA8888,
                             (void*)cursor->pixel_data,
...
void OGC_InitMouse(SDL_VideoDevice *_this)
{
    ...
    SDL_SetDefaultCursor(OGC_CreateSystemCursor(SDL_SYSTEM_CURSOR_POINTER));
}
...
void OGC_draw_cursor(SDL_VideoDevice *_this)
{
    ...
    if (s_extra_draw_enabled) {
        ...
        x = (s16)mouse->x - radius;
        y = (s16)mouse->y - radius;
...
void OGC_restore_viewport(SDL_VideoDevice *_this)
{
    ...
    if (_this->windows) {
        ...
        if (renderer) {
            OGC_set_viewport(renderer->main_view.viewport.x, renderer->main_view.viewport.y,
                             renderer->main_view.viewport.w, renderer->main_view.viewport.h);
        }
    }
}
