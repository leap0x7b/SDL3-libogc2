...
static bool OGC_VideoInit(SDL_VideoDevice *_this);
static void OGC_VideoQuit(SDL_VideoDevice *_this);

static void init_display_mode(SDL_DisplayMode *mode, const GXRModeObj *vmode)
{
    SDL_DisplayModeData *modedata = SDL_malloc(sizeof(SDL_DisplayModeData));
    ...
    modedata->vmode = (GXRModeObj*)vmode;
...
static void setup_video_mode(SDL_VideoDevice *_this, GXRModeObj *vmode)
{
...
static bool OGC_SetDisplayMode(SDL_VideoDevice *_this, SDL_VideoDisplay *display,
                                SDL_DisplayMode *mode)
{
    SDL_VideoData *videodata = (SDL_VideoData *)_this->internal;
...
static void OGC_ShowWindow(SDL_VideoDevice *_this, SDL_Window *window)
{
    SDL_SetMouseFocus(window);
    SDL_SetKeyboardFocus(window);
}

static bool OGC_ShowMessageBox(SDL_VideoDevice *_this, const SDL_MessageBoxData *messageboxdata,
                                int *buttonid)
{
...
static SDL_VideoDevice *OGC_CreateDevice(void)
{
    ...
    device->GL_DestroyContext = SDL_OGC_GL_DestroyContext;
...
bool OGC_VideoInit(SDL_VideoDevice *_this)
{
    SDL_VideoData *videodata = (SDL_VideoData *)_this->internal;
    SDL_DisplayMode mode;
    GXRModeObj *vmode;
    ...
    SDL_AddFullscreenDisplayMode(_this->displays[0], &mode);
...
void OGC_VideoQuit(SDL_VideoDevice *_this)
{
    SDL_VideoData *videodata = (SDL_VideoData *)_this->internal;
     ...
     * crash, since we didn't actually allocate this memory. */
     ...
     for (int i = display->num_fullscreen_modes; i--;) {
         display->fullscreen_modes[i].internal = NULL;
     }
     display->desktop_mode.internal = NULL;
}

void *OGC_video_get_xfb(SDL_VideoDevice *_this)
{
    SDL_VideoData *videodata = _this->internal;
    return videodata->xfb[videodata->fb_index];
}

void OGC_video_flip(SDL_VideoDevice *_this, bool vsync)
{
    SDL_VideoData *videodata = _this->internal;
